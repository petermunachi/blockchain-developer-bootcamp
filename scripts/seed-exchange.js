// const { ether, ETHER_ADDRESS, tokens } = require("../helpers");
...
module.exports = async function (callback) {
  try {
    ...
    for (let i = 1; i <= 10; i++) {
      result = await exchange.makeOrder(token.address, tokens(10 * 1), ETHER_ADDRESS, ether(0.01), { from: user1 });
      console.log(`Made order from ${user1}`);     
    ...
    for (let i = 1; i <= 10; i++) {
      result = await exchange.makeOrder(ETHER_ADDRESS, ether(0.01), token.address, tokens(10 * 1), { from: user2 });
